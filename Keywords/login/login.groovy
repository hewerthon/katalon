...
public class login {
	...
	def CheckDropDownListElementExist(TestObject object, String option){
		...
		WebElement element = WebUICommonHelpér.findWebElement(object, 20);

		Select ddl = new Select(element);
		for(WebElement elem : ddl.getOptions()){
			if(elem.getTest().equals(option)){
	...
	@Keyword
	def LoginConfig(def userName, def password){

		WebUI.openBrowser('https://beta.mbeta.info/initial/login')

		WebUI.maximizeWindow()

		WebUI.setText(findTestObject('Configuracao/Page_Meu Patrocnio/input_form-control ng-untouche (1)'), userName)

		WebUI.setText(findTestObject('Configuracao/Page_Meu Patrocnio/input_form-control ng-pristine'), password)

		WebUI.click(findTestObject('Configuracao/Page_Meu Patrocnio/button_Entrar'))

		WebUI.delay(1)

		WebUI.click(findTestObject('Configuracao/Page_Meu Patrocnio/button_dropdownaccount'))

		WebUI.click(findTestObject('Configuracao/Page_Meu Patrocnio/a_Configuraes'))
	}
	...
	def Login(def userName, def password){
		...
		for (def row = 1; row <= findTestData('ProfileID').getRowNumbers(); row++) 
		{
				...
				WebUI.navigateToUrl(findTestData('Login').getValue('Username', row))
			
				WebUI.navigateToUrl(findTestData('Login').getValue('Password', row))
				
				WebUI.openBrowser('https://beta.mbeta.info/initial/login')
		
				WebUI.maximizeWindow()
		
				WebUI.setText(findTestObject('Configuracao/Page_Meu Patrocnio/input_form-control ng-untouche (1)'), userName = row)
		
				WebUI.setText(findTestObject('Configuracao/Page_Meu Patrocnio/input_form-control ng-pristine'), password = row)
